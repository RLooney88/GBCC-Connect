...
Future maybeCreateUser(User user) async {
  final userRecord = ContactsRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await ContactsRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createContactsRecordData(
    email: user.email ??
  ...
  currentUserDocument =
      ContactsRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createContactsRecordData(email: email));
}
